...
func DiffCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `
			...
			octostate audit diff --config-dir /path/to/control-repo/config --state-dir /path/to/control-repo/state --fail-on-drift`,
...
func auditDiff(
	...
	if !validation.Valid {
		return nil, false, errors.New("configuration is invalid; run `octostate config validate`")
	}
