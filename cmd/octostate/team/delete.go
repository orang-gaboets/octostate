...
func deleteTeamToConfig(cmd *cobra.Command, path, org, slug string) error {
	changed, err := configproposal.ApplyToConfigFile(path, org, func(cfg *gitopsconfig.OrganizationConfig) error {
		...
		blockers := collectTeamDeleteBlockers(cfg, slug)
		if len(blockers) > 0 {
			return fmt.Errorf(
				"team %s/%s cannot be deleted from config while dependencies exist: %s",
				org,
				slug,
...
func collectTeamDeleteBlockers(cfg *gitopsconfig.OrganizationConfig, slug string) []string {
	if cfg == nil {
		return nil
	}
