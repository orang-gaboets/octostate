...
func TestPrintCheckOmitsNilData(t *testing.T) {
	cmd := &cobra.Command{}
	var out bytes.Buffer
	cmd.SetOut(&out)

	if err := PrintCheck(cmd, "checked repository apply", nil); err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	...
	if got["status"] != string(OperationResultStatusCheck) {
		t.Fatalf("expected status %q, got %#v", OperationResultStatusCheck, got["status"])
	}
	if got["message"] != "checked repository apply" {
		t.Fatalf("expected message %q, got %#v", "checked repository apply", got["message"])
	}
	if _, exists := got["data"]; exists {
		t.Fatalf("expected omitted data field, got %#v", got["data"])
	}
