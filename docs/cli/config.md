...
- Managed same-organization repository dependencies are emitted in deterministic dependency-first DFS postorder, so a source update or create precedes its consumers
- Existing sources use their explicit final `is_template` value, or retain live state when the field is omitted; new sources are usable only with `is_template: true`. Unavailable sources propagate diagnostics transitively, and cycles report stable `template dependency cycle: ...` messages
...
- `--check` is a best-effort preflight: it continues through independent actions and aggregates failures in plan order. It validates the supported apply executor inputs plus live read probes, but it is not a guaranteed GitHub transaction dry-run
