...
| `dynamic/github-code-scanning/codeql` — GitHub-managed | Pull requests, pushes to the default branch, and scheduled runs, as observed in recent runs | Workflow metadata may say `CodeQL`; run labels are `Code Quality: PR #...`, `Code Quality: Push on main`, or `Code Quality: Scheduled`; job `Analyze (go)` | CodeQL-powered Code Quality analysis; default-branch findings appear under `Security and quality → Standard findings`; pull-request findings appear as comments from `github-code-quality[bot]` and the `CodeQL - Code Quality / Analyze` check | GitHub repository Code Quality / Advanced Security settings; no generated workflow file is tracked here |
