...
func (b builder) planTeams() []Action {
	...
	actualTeams := make(map[string]state.Team, len(b.actual.Teams))
	for _, team := range b.actual.Teams {
		actualTeams[teamKey(team.Slug)] = team
	}

	desiredTeams := make(map[string]config.TeamSpec, len(b.desired.Teams))
	...
	for _, team := range b.actual.Teams {
		...
		actions = append(actions, Action{
			...
			ResourceID:   teamID(team.Slug),
			Executable:   false,
			Message:      fmt.Sprintf("team %s exists in snapshot state but is not declared in desired config", teamID(team.Slug)),
		})
	}
...
func (b builder) planTeamMembers() []Action {
	actions := make([]Action, 0)
	actualMembers := make(map[string]state.TeamMember, len(b.actual.TeamMembers))
	...
	for _, member := range b.actual.TeamMembers {
		...
		actions = append(actions, Action{
			...
			Executable:   false,
			Message:      fmt.Sprintf("team membership %s exists in snapshot state but is not declared in desired config", teamMemberID(member.TeamSlug, member.Username)),
		})
	}
...
func (b builder) planTeamRepositoryPermissions() []Action {
	...
	repositoryAvailabilityForPermission := func(owner, name string) (bool, string) {
		repositoryID := repositoryID(owner, name)
		...
		if !ok {
			return false, fmt.Sprintf("repository %s is absent from snapshot and desired state", repositoryID)
		}
		if strings.TrimSpace(desiredRepository.Template.Owner) == "" || strings.TrimSpace(desiredRepository.Template.Name) == "" {
			return false, fmt.Sprintf("repository %s is declared in desired state but is not creatable: template configuration is missing", repositoryID)
		}
		return true, ""
	}
	...
	for _, permission := range b.actual.TeamRepositoryPermissions {
		...
		actions = append(actions, Action{
			...
			Executable:   false,
			Message:      fmt.Sprintf("team repository permission %s exists in snapshot state but is not declared in desired config", teamRepositoryPermissionID(permission.TeamSlug, permission.Owner, permission.Name)),
		})
	}
