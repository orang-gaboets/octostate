...
func (b builder) planOrganizationMembers() []Action {
	actions := make([]Action, 0)

	actualMembers := make(map[string]state.OrganizationMember, len(b.actual.Members))
	for _, member := range b.actual.Members {
		actualMembers[organizationMemberKey(member.Username)] = member
	}

	desiredMembers := make(map[string]config.OrganizationMemberSpec, len(b.desired.Members))
	...
	for _, member := range b.actual.Members {
		...
		actions = append(actions, Action{
			...
			ResourceID:   organizationMemberID(member.Username),
			Executable:   false,
			Message:      fmt.Sprintf("organization member %s exists in snapshot state but is not declared in desired config", organizationMemberID(member.Username)),
		})
	}
